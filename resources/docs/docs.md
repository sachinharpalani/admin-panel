...
The `home-routes` handler in the `admin-panel.routes.home` namespace
...
The `home-page` function will in turn call the `admin-panel.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/admin_panel/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `admin-panel.handler` namespace:
...
Request middleware functions are located under the `admin-panel.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `admin-panel.dev-middleware` namespace found in
